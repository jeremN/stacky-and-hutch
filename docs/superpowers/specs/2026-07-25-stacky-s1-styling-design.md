...
- **Iconify has no explicit `[requires]`.** Its point-based injects synthesize
  ...
  by a web framework — so iconify **transitively requires a framework**. Because
  `app-head`/`app-shell` each have two candidate publishers (both frameworks),
  adding a styling brick with no framework → `ambiguous-injection-point` (exit 2,
  "pick a framework": `candidates: ['sveltekit', 'tanstack-start']`), not
  unsatisfiable. Free, from the phase-2a mechanism.
...
6. **Ambiguity safety.** Adding a styling brick (`tailwind`/`iconify`) with no
   framework resolves to `ambiguous-injection-point` (exit 2) — because
   `app-head` has *two* candidate publishers (both frameworks), the resolver
   reports "pick a framework" (`candidates: ['sveltekit', 'tanstack-start']`)
   rather than "unsatisfiable". Asserted as data.
